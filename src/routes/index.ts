...
최대 “2,000원” 까지 포인트 지급해드려요.`;
    ...
    try {
      ...
      if(existUser['cnt'] == 0 || existUserInfo['cnt'] != 0) {
        ...
        toUserMsg  = `불편 접수 완료!
현재 고객님의 포인트 : ${totalPoint['point_total']}원
(어뷰징으로 판단될 경우,
포인트는 회수될 수 있으니 참고 부탁드립니다.)`;
...
router.post('/kakaoChat/myRefCode', async (ctx, next) => {
  ...
  if(existUser['cnt'] == 0 || existUserInfo['cnt'] != 0) {
    ...
    toUserMsg = `(너의 불편이 곧, 나의 불편)
우리의 불편이 가치가 되는 순간!
지금 바로 친구에게 “프로불편러” 채널을 공유하고
혜택 받아가세요.
📍친구가 추천인 코드 입력시,
추천인 - 1000원 적립
추천받은 친구 - 500원 적립   
...
🔐 추천인 코드 : 
프로불편러 채널 : 링크삽입 => 버튼으로 대체 가능?
    
당신이 일상 속에서 마주하는 불편을
“프로불편러”에게 공유해주세요!
작성하신 모든 분들 께는 최대 “2000원”까지
보상해 드려요
    
불편이 떠오르지 않나요?
어떠한 제품이나 서비스의 제안과 필요한 이유를
작성해주셔도 좋아요
    
더 많은 공감을 받은 불편은 
이를 잘 해결할 수 있는 대학, 기관, 팀 등에게
전달되어, 세상을 조금 더 편하게 바꾸는데 활용됩니다.
